...
namespace UnrealMcp
{
	...
	FString GetAssistantSafetyRulesBlock()
	{
		...
		Rules.Add(TEXT("3. AI self-extension is the project-local Python user-tool track ONLY (scaffold_mcp_tool python -> mcp_user_registry_reload -> mcp_user_tool_smoke -> usable). Core C++ apply/promotion is hidden, manual, developer-only — never call or route to mcp_apply_scaffold / mcp_extension_pipeline (they are not in your tool surface), and NEVER use execute_python / execute_python_file to modify plugin source under Plugins/UnrealMcp/Source or the ToolRegistry JSON."));
