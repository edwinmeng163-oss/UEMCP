...
namespace UnrealMcpPythonToolBridge
{
	namespace
	{
		...
		FString BuildPythonWrapperCommand(const FString& ParentDirectory, const FString& ArgumentsJson, const TArray<FString>& AllowList)
		{
			return FString::Printf(
				...
				TEXT("try:\n")
				TEXT("    import unreal\n")
				TEXT("    def _unreal_mcp_call_tool_raw(name, args=None):\n")
				TEXT("        _aj = json.dumps(args or {})\n")
				TEXT("        _raw = unreal.UnrealMcpCallTool.call_tool(name, _aj)\n")
				TEXT("        return json.loads(_raw)\n")
				TEXT("    def _unreal_mcp_call_tool(name, args=None):\n")
				TEXT("        _r = _unreal_mcp_call_tool_raw(name, args)\n")
				TEXT("        if _r.get('isError'):\n")
				TEXT("            raise RuntimeError(_r.get('meta', {}).get('reason') or _r.get('text') or 'call_tool failed')\n")
				TEXT("        return _r\n")
				TEXT("    builtins.call_tool = _unreal_mcp_call_tool\n")
				TEXT("    builtins.call_tool_raw = _unreal_mcp_call_tool_raw\n")
				...
				TEXT("finally:\n")
				TEXT("    builtins.__dict__.pop('call_tool', None)\n")
				TEXT("    builtins.__dict__.pop('call_tool_raw', None)\n")
	...
	FUnrealMcpExecutionResult ExecutePythonRegisteredTool(const FToolHandlerRegistryEntry& HandlerEntry, const FJsonObject& Arguments)
	{
		...
		const FString ActualSha256 = UnrealMcp::HashUtils::Sha256LowerHex(HandlerBytes).ToLower();
