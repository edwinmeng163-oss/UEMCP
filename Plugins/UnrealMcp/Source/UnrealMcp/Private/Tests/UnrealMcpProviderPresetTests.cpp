...
#include "Providers/CodexProviderShellHelpers.h"
...
bool FUnrealMcpCodexProviderSubprocessPathPrefixTest::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("Prefix MUST contain zero literal double-quote chars (UE CreateProc tokenizer invariant)."), Prefix.Contains(TEXT("\"")));
	TestTrue(TEXT("Prefix uses bare PATH= assignment (no 'export' builtin)."), Prefix.StartsWith(TEXT("PATH=")));
	TestFalse(TEXT("Prefix MUST NOT use 'export PATH=' form (word-splits inherited $PATH containing spaced dirs like /Library/Application Support/...)."), Prefix.Contains(TEXT("export")));
	TestFalse(TEXT("Prefix has no literal ASCII space."), Prefix.Contains(TEXT(" ")));

	return true;
}

namespace UnrealMcpCodexProviderShellIntegrationTests
{
	bool WriteArgcArgvPrinterScript(FString& OutScriptPath, FString& OutError)
	{
		const FString TempDir = FPaths::Combine(FPaths::ProjectIntermediateDir(), TEXT("UnrealMcpTests"));
		IFileManager::Get().MakeDirectory(*TempDir, true);
		OutScriptPath = FPaths::Combine(
			TempDir,
			FString::Printf(TEXT("argcargv-%s.sh"), *FGuid::NewGuid().ToString(EGuidFormats::DigitsLower)));

		const FString ScriptBody = TEXT(
			"#!/bin/sh\n"
			"printf 'ARGC=%d\\n' \"$#\"\n"
			"for arg do\n"
			"    printf 'ARG=%s\\n' \"$arg\"\n"
		...
		{
			OutError = FString::Printf(TEXT("Failed to write helper script: %s"), *OutScriptPath);
			return false;
		}

#if PLATFORM_MAC || PLATFORM_LINUX
		if (::chmod(TCHAR_TO_UTF8(*OutScriptPath), 0755) != 0)
		{
			OutError = FString::Printf(TEXT("Failed to chmod helper script: %s"), *OutScriptPath);
			return false;
		}
#endif
	...
	bool RunBashCMinusC(const FString& Arguments, FString& OutStdout, int32& OutExitCode, FString& OutError)
	{
		void* ReadPipe = nullptr;
		void* WritePipe = nullptr;
		if (!FPlatformProcess::CreatePipe(ReadPipe, WritePipe))
		{
			OutError = TEXT("Failed to create pipe.");
			return false;
		}
		...
		while (FPlatformProcess::IsProcRunning(Proc))
		{
			...
			if (FPlatformTime::Seconds() - StartTime > TimeoutSeconds)
			{
				...
				OutError = TEXT("Subprocess timed out.");
				return false;
			}
	...
	int32 CountLiteralSpaces(const FString& Value)
	{
		int32 SpaceCount = 0;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FUnrealMcpCodexProviderSpawnedCommandActualBashExecTest,
	"UnrealMcp.CodexProvider.SpawnedCommandActualBashExec",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FUnrealMcpCodexProviderSpawnedCommandActualBashExecTest::RunTest(const FString& Parameters)
{
	using namespace UnrealMcpCodexProviderShellIntegrationTests;
	(void)Parameters;

	if (!FPaths::FileExists(TEXT("/bin/bash")))
	{
		AddInfo(TEXT("/bin/bash not present; skipping integration test."));
		return true;
	}

	FString HelperScript;
	FString WriteErr;
	if (!WriteArgcArgvPrinterScript(HelperScript, WriteErr))
	{
		AddError(WriteErr);
		return false;
	}
	ON_SCOPE_EXIT { IFileManager::Get().Delete(*HelperScript, false, true, true); };

	const FString TestPrompt = TEXT("hello world\n'with' \"embedded\"\ttab and newline");
	const FString TestContext = TEXT("prior context line one\nprior context line two");
	const FString ComposedPrompt = UnrealMcp::Providers::Internal::ComposePrompt(TestPrompt, TestContext);

	const FString FakeCodexBinary = HelperScript;
	const TArray<FString> NoExtraArgs;
	...
	TestEqual(TEXT("Arguments has exactly 1 literal ASCII space."), CountLiteralSpaces(Arguments), 1);
	TestFalse(TEXT("Arguments contains zero literal double-quote chars."), Arguments.Contains(TEXT("\"")));

	FString Stdout;
	int32 ExitCode = -1;
	FString RunErr;
	if (!RunBashCMinusC(Arguments, Stdout, ExitCode, RunErr))
	{
		AddError(FString::Printf(TEXT("bash -c failed to launch/run: %s"), *RunErr));
		return false;
	}

	TestEqual(TEXT("bash -c exit code 0."), ExitCode, 0);
	TestTrue(TEXT("Stdout contains ARGC=12."), Stdout.Contains(TEXT("ARGC=12\n")));

	const TArray<FString> ExpectedArgs =
	{
		TEXT("start"),
		ComposedPrompt,
		TEXT("-m"),
	...
	for (const FString& ExpectedArg : ExpectedArgs)
	{
		const FString ExpectedArgLine = FString::Printf(TEXT("ARG=%s"), *ExpectedArg);
		TestTrue(
			FString::Printf(TEXT("Stdout contains argv value '%s'."), *ExpectedArg.Left(40)),
			Stdout.Contains(ExpectedArgLine));
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FUnrealMcpCodexProviderKillCommandActualBashExecTest,
	"UnrealMcp.CodexProvider.KillCommandActualBashExec",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FUnrealMcpCodexProviderKillCommandActualBashExecTest::RunTest(const FString& Parameters)
{
	using namespace UnrealMcpCodexProviderShellIntegrationTests;
	(void)Parameters;
	...
	const FString FakeJobId = TEXT("test-job-001");
	const FString FakeCodexBinary = HelperScript;
	const FString ProjectDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectDir());
	...
	TestEqual(TEXT("KillArguments has exactly 1 literal ASCII space."), CountLiteralSpaces(KillArguments), 1);
	TestFalse(TEXT("KillArguments contains zero literal double-quote chars."), KillArguments.Contains(TEXT("\"")));

	FString Stdout;
	int32 ExitCode = -1;
	FString RunErr;
	if (!RunBashCMinusC(KillArguments, Stdout, ExitCode, RunErr))
	{
		AddError(FString::Printf(TEXT("bash -c failed: %s"), *RunErr));
		return false;
	}

	TestEqual(TEXT("bash -c exit code 0."), ExitCode, 0);
	TestTrue(TEXT("Stdout contains ARGC=2."), Stdout.Contains(TEXT("ARGC=2\n")));
	TestTrue(TEXT("Stdout contains ARG=kill."), Stdout.Contains(TEXT("ARG=kill\n")));
	TestTrue(TEXT("Stdout contains ARG=<JobId>."), Stdout.Contains(FString::Printf(TEXT("ARG=%s\n"), *FakeJobId)));
