...
bool FUnrealMcpProviderPresetRegistryTest::RunTest(const FString& Parameters)
{
	...
	if (CodexCliPreset)
	{
		...
		TestTrue(TEXT("Codex CLI preset leaves CodexBinaryPath empty."), CodexCliPreset->CodexBinaryPath.IsEmpty());
		TestTrue(TEXT("Codex CLI preset leaves CodexExtraArgs empty."), CodexCliPreset->CodexExtraArgs.IsEmpty());
	}
