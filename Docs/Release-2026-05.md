...
## v0.28.0 — 2026-05-24 — core Blueprint authoring + playable-character closure + runtime verification

### 中文

v0.28.0 补齐核心蓝图创作层并新增运行时验证,让 AI 不依赖 `execute_python` 就能造出真正可玩的第三人称角色。新增 10 个核心工具:`bp_add_input_axis_event_node` / `bp_add_input_action_event_node`(Legacy 输入事件节点)、`bp_add_component` / `bp_set_component_property`(蓝图 SCS 组件:SpringArm/Camera/任意 SceneComponent + 带 allowlist 的属性设置)、`bp_set_class_default`(类默认值,如 GameMode.DefaultPawnClass)、`editor_set_map_game_mode`(当前地图 GameMode override)、`actor_set_auto_possess`、`bp_add_gameplay_node`(枚举式常用 gameplay 节点:AddMovementInput / AddControllerYawInput / AddControllerPitchInput / GetActorForwardVector / GetActorRightVector / GetControlRotation / Jump / FloatGreaterThan)、`pie_input_probe`(在 PIE 中异步注入输入并量测 pawn 位移,FTSTicker 驱动、绝不从 handler 泵编辑器 tick)、`verify_viewport_widgets`(只读检查 viewport 上的 UMG widget)。扩展 `configure_player_input` 接受任意 Legacy 轴/动作映射名;修复 `bp_set_pin_default` 对 object/class 默认引脚的 postcheck 误报。大量复用既有的 `UnrealMcpBlueprintGraphLibrary`。并入 v0.27.1 的 `verify_player_controls` 崩溃修复(不再从 handler 重入编辑器 tick)。工具总数 164→174。实测:AI 仅用可见核心工具(无 `execute_python`)造出可玩第三人称角色,`pie_input_probe` 量得前进约 126cm(经输入层注入触发绑定图)。已知限制:`pie_input_probe` 对 InputAction(跳跃)目前退回直接调用(弱证明);`AutomationTools.RunRecoversStale`(自 v0.20)与 `PieSmoke.MapValidation` 为 pre-existing/flaky,与本版无关。双引擎 UE 5.6 + 5.7。

### EN

v0.28.0 fills the core Blueprint authoring layer and adds runtime verification, so the AI can build a genuinely playable third-person character without falling back to `execute_python`. Ten new core tools: `bp_add_input_axis_event_node` / `bp_add_input_action_event_node` (legacy input-event nodes), `bp_add_component` / `bp_set_component_property` (Blueprint SCS components — SpringArm/Camera/any SceneComponent + allowlisted property set), `bp_set_class_default` (class defaults like GameMode.DefaultPawnClass), `editor_set_map_game_mode` (current-map GameMode override), `actor_set_auto_possess`, `bp_add_gameplay_node` (enum of common gameplay nodes: AddMovementInput / AddControllerYawInput / AddControllerPitchInput / GetActorForwardVector / GetActorRightVector / GetControlRotation / Jump / FloatGreaterThan), `pie_input_probe` (async input injection + pawn-displacement measurement in PIE, FTSTicker-driven, never pumps the editor tick from the handler), and `verify_viewport_widgets` (read-only check of UMG widgets on the viewport). `configure_player_input` is extended to accept arbitrary legacy axis/action mapping names; `bp_set_pin_default`'s postcheck for object/class default pins is fixed. Heavy reuse of the existing `UnrealMcpBlueprintGraphLibrary`. Folds in the v0.27.1 `verify_player_controls` crash fix (no longer re-enters the editor tick from the handler). Tool count 164→174. Verified: the AI builds a playable third-person character using only visible core tools (no `execute_python`), and `pie_input_probe` measured ~126cm of forward movement (input-layer injection driving the bound graph). Known limitations: `pie_input_probe` currently falls back to a direct call for InputAction (jump) — weaker proof; `AutomationTools.RunRecoversStale` (since v0.20) and `PieSmoke.MapValidation` are pre-existing/flaky and unrelated to this release. Dual-engine UE 5.6 + 5.7.

### 日本語

v0.28.0 は core Blueprint authoring layer を補完し runtime verification を追加します。AI は `execute_python` に頼らず、真に playable な third-person character を構築できます。新しい core tool を 10 個追加：`bp_add_input_axis_event_node` / `bp_add_input_action_event_node`（legacy input-event node）、`bp_add_component` / `bp_set_component_property`（Blueprint SCS component — SpringArm/Camera/任意の SceneComponent + allowlist 付き property 設定）、`bp_set_class_default`（GameMode.DefaultPawnClass などの class default）、`editor_set_map_game_mode`（現在マップの GameMode override）、`actor_set_auto_possess`、`bp_add_gameplay_node`（よく使う gameplay node の enum：AddMovementInput / AddControllerYawInput / AddControllerPitchInput / GetActorForwardVector / GetActorRightVector / GetControlRotation / Jump / FloatGreaterThan）、`pie_input_probe`（PIE で input を非同期注入し pawn の変位を計測、FTSTicker 駆動で handler から editor tick を pump しない）、`verify_viewport_widgets`（viewport 上の UMG widget を read-only で確認）。`configure_player_input` を任意の legacy axis/action マッピング名を受け付けるよう拡張；`bp_set_pin_default` の object/class default pin に対する postcheck 誤検知を修正。既存の `UnrealMcpBlueprintGraphLibrary` を大幅に再利用。v0.27.1 の `verify_player_controls` クラッシュ修正（handler から editor tick に再入しない）を統合。tool 総数 164→174。検証：AI は visible core tool のみ（`execute_python` なし）で playable な third-person character を構築し、`pie_input_probe` で前進約 126cm を計測（input layer 注入が bound graph を駆動）。既知の制限：`pie_input_probe` は InputAction（jump）に対して現状 direct call に fallback（弱い証明）；`AutomationTools.RunRecoversStale`（v0.20 以降）と `PieSmoke.MapValidation` は pre-existing/flaky で本リリースと無関係。dual-engine UE 5.6 + 5.7。
