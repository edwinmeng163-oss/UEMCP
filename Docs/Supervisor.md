...
By default, the repository supervisor targets the root local development host at `UEvolve.uproject`. Pass `--uproject /path/to/YourProject.uproject` when using the plugin inside another Unreal project, or pick one of the bundled samples: `--uproject Examples/UEvolveExample/UEvolveExample.uproject` (UE 5.6.1) or `--uproject Examples/UEvolveExample57/UEvolveExample57.uproject` (UE 5.7.4).
