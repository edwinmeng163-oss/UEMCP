...
ratings, pinning, promotion to skills/RAG, Make Tool Set composites, or
...
- `STaskAtlasWindow`: Chat-launched Slate view for workflows, unused tools,
  Task Atlas-made user tools, search, tool details, pinning, Distill Skill,
  To RAG, and Make Tool Set.
...
`Make Tool Set` now derives a `user.atlas_*` tool name from the workflow label,
...
The Made Tools panel lists only user-registry entries whose `tool.json`
declares `generator=task-atlas-composite`; deletion confirms with the user,
checks the scaffold directory remains under `GetUserToolsRootDir()`, deletes
that directory, then reloads the user registry.
...
`Make Tool Set` derives an `atlas_*` user-tool id from the workflow label and writes
...
For AI or user review, treat `Make Tool Set` output as a draft:
