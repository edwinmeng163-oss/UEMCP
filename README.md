...
- Reform C centralizes the assistant safety prompt, adds the approval gate, and makes project-local Python user extensions the default self-extension path.
...
- Reform C 统一 assistant 安全 system prompt，加入 approval gate，并把 project-local Python user extension 作为默认自扩展路径。
...
- Reform C では assistant safety system prompt を一元化し、approval gate を追加し、project-local Python user extension を default self-extension path にしました。
...
- `Plugins/UnrealMcp`, an editor plugin for local MCP and in-editor AI/chat workflows (`VersionName: 0.26.0`).
