...
Latest release: **[v0.26.0 — Reform C safety prompt, approval gate, and 162-tool registry](https://github.com/edwinmeng163-oss/UEAtelier/releases/tag/v0.26.0)** (public, source-only, ~1 MiB each). Mac and Windows zips both attached; same source compiles cleanly on UE 5.6 and UE 5.7. Extract the projectroot zip next to your `<YourProject>.uproject` (NOT under `Plugins/`); UBT compiles on first editor launch.
...
最新 release：**[v0.26.0 — Reform C 安全提示、approval gate 和 162 工具注册表](https://github.com/edwinmeng163-oss/UEAtelier/releases/tag/v0.26.0)**（公开、source-only、各约 1 MiB）。Mac 和 Windows zip 都已上传；同一份 source 在 UE 5.6 和 UE 5.7 上都能干净编译。把 projectroot zip 解压到 `<YourProject>.uproject` 同级目录（不要解到 `Plugins/` 下），UBT 在首次启动编辑器时自动编译。
...
- `Plugins/UnrealMcp`, an editor plugin for local MCP and in-editor AI/chat workflows (`VersionName: 0.27.0`).
...
- PIE start/stop.
- Actor selection, readback inspectors, transforms, spawning, layout, auto-possess setup, and batch property edits.
- Project settings readback through `unreal.project_settings_get`.
- Player input setup with standard or arbitrary legacy names, map GameMode assignment, control existence verification, PIE input movement probes, and viewport widget verification through `unreal.configure_player_input`, `unreal.editor_set_map_game_mode`, `unreal.verify_player_controls`, `unreal.pie_input_probe`, and `unreal.verify_viewport_widgets`.
