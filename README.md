...
- The Chat panel `Task Atlas` button opens a Slate workflow view backed by `Saved/UnrealMcp/Tasks`. v0.17 supports local task extraction, search, detail inspection, rating, and pinning. v0.18 makes `To Skills` and `To RAG` functional for selected workflows; `Make Tool` remains a visible v0.19 placeholder.
