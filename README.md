...
Latest release: **[v0.28.0 — core Blueprint authoring, playable-character closure, runtime verification, and 174-tool registry](https://github.com/edwinmeng163-oss/UEAtelier/releases/tag/v0.28.0)** (public, source-only projectroot zips, ~0.8 MiB each). Mac and Windows zips are both attached; the same source compiles cleanly on UE 5.6 and UE 5.7. Extract the projectroot zip next to your `<YourProject>.uproject` (NOT under `Plugins/`); UBT compiles on first editor launch or explicit editor-target build.
...
- 181 registered MCP tools across actors, blueprint, code, editor, material, memory, scaffold, self-extension, skills, task-atlas, verification, and widget categories.
...
- Runtime verification includes `unreal.pie_input_probe` and `unreal.verify_viewport_widgets`; `unreal.configure_player_input` accepts arbitrary legacy axis/action names, and `unreal.bp_set_pin_default` has corrected object/class pin postchecks.
- v0.27 makes AI self-extension the project-local Python user-tool path only; core apply/pipeline and promotion are hidden, manual, developer-only, and deferred.
...
最新 release：**[v0.28.0 — 核心蓝图创作、可玩角色闭环、运行时验证和 174 工具注册表](https://github.com/edwinmeng163-oss/UEAtelier/releases/tag/v0.28.0)**（公开、source-only projectroot zip、各约 0.8 MiB）。Mac 和 Windows zip 都已上传；同一份 source 在 UE 5.6 和 UE 5.7 上都能干净编译。把 projectroot zip 解压到 `<YourProject>.uproject` 同级目录（不要解到 `Plugins/` 下），UBT 会在首次启动编辑器或显式构建 editor target 时编译插件。
...
- 已注册 181 个 MCP tools，覆盖 actors、blueprint、code、editor、material、memory、scaffold、self-extension、skills、task-atlas、verification、widget 分类。
...
- 运行时验证包含 `unreal.pie_input_probe` 与 `unreal.verify_viewport_widgets`；`unreal.configure_player_input` 接受任意 legacy 轴/动作映射名，`unreal.bp_set_pin_default` 也修复了 object/class 引脚 postcheck。
- v0.27 将 AI 自扩展限定为 project-local Python user-tool 路径；core apply/pipeline 与 promotion 均为 hidden、manual、developer-only，并继续 deferred。
...
- actors、blueprint、code、editor、material、memory、scaffold、self-extension、skills、task-atlas、verification、widget カテゴリにまたがる 181 個の MCP tools を登録済みです。
...
- runtime verification には `unreal.pie_input_probe` と `unreal.verify_viewport_widgets` が含まれます。`unreal.configure_player_input` は任意の legacy axis/action mapping name を受け付け、`unreal.bp_set_pin_default` は object/class pin postcheck を修正済みです。
- v0.27 では AI self-extension を project-local Python user-tool path のみに限定し、core apply/pipeline と promotion は hidden / manual / developer-only / deferred にしました。
...
UEAtelier is listed on the public [CLI-Anything Hub registry](https://github.com/HKUDS/CLI-Anything/blob/main/registry.json) (PR HKUDS/CLI-Anything#297 merged 2026-05-20 in commit [48e76a0](https://github.com/HKUDS/CLI-Anything/commit/48e76a0bca9616b6e60657187baa81164c4894cd)). Full command reference is in [SKILL.md](Tools/UEAtelierCli/SKILL.md).
...
- Player input setup with standard or arbitrary legacy names, map GameMode assignment, control existence verification, PIE input movement probes, and viewport widget verification through `unreal.configure_player_input`, `unreal.editor_set_map_game_mode`, `unreal.actor_set_auto_possess`, `unreal.verify_player_controls`, `unreal.pie_input_probe`, and `unreal.verify_viewport_widgets`.
- Python execution inside Unreal Editor.
- Blueprint class creation, compilation, class defaults, SCS component authoring, legacy input events, and curated gameplay graph node authoring.
...
- Developer-only core extension integration tools are retained for manual use
  but hidden from AI-facing `tools/list`:
...
For AI-created MCP tools, use the project-local Python user-tool track:

```text
/tool unreal.scaffold_mcp_tool {"toolName":"unreal.my_custom_tool","title":"My Custom Tool","description":"Scaffold a new MCP user tool.","implementationTrack":"python","exampleArgumentsJson":"{\"message\":\"hello\"}"}
/tool unreal.mcp_user_registry_reload {}
/tool unreal.mcp_user_tool_smoke {"toolName":"unreal.my_custom_tool"}
```

The tool is usable only after the reload/smoke result reports
`structuredContent.lifecycle.callableNow=true`. Python user tools are
project-local under:

```text
Tools/UnrealMcpPyTools/<tool_id>/main.py
```

Core C++ apply/pipeline tools still exist for manual developer work, but they
are hidden from the AI tool surface. The AI should not call or route through
`unreal.mcp_apply_scaffold` or `unreal.mcp_extension_pipeline`.

Useful AI-facing extension checks:
...
- `unreal.mcp_validate_cpp_patch` statically checks generated C++ patch fragments for risky operations before manual developer source integration, including process execution, destructive file operations, recursive pipeline calls, obvious infinite loops, missing handler returns, and flexible schema warnings.
...
- `unreal.mcp_extension_pipeline` is a developer-only hidden orchestration tool for validate, test generation, apply dry run, apply, memory write, build, restart handoff, and post-restart test suite resume.
...
- `unreal.workflow_run` is the generic composition executor. It accepts inline `steps`, a `workflowJson` string, or a project-local `workflowPath`, defaults to `dryRun:true`, blocks nested workflows, rejects hidden tools, blocks high/critical step tools unless explicitly allowed, executes every remaining step through the normal MCP handler path, and can write `chat.active_task` for planned/paused/completed workflows.
- `Window > UEAtelier Workbench` provides a thin Slate control panel over existing MCP tools. It can refresh workbench status, run audit, run the versioned core tests, inspect pipeline/lock state, inspect Skill Activity, distill a draft, run promote dry-runs, and copy the last structured result without duplicating backend logic.
- The Chat panel `Task Atlas` button opens a Slate workflow view backed by `Saved/UnrealMcp/Tasks`. v0.17 supports local task extraction, search, detail inspection, rating, and pinning. v0.18 makes `To Skills` and `To RAG` functional for selected workflows. v0.19 completes Make Tool scaffold creation, Task Atlas markdown RAG ingestion, and conservative LLM label backfill for placeholder workflow labels.
...
flowchart LR
    ...
    F --> G["Open Window > UEAtelier Chat"]
...
```powershell
git clone https://github.com/edwinmeng163-oss/UEAtelier.git C:\UnrealProjects\UEAtelier
...
1. Clone and enter the repository: `git clone https://github.com/edwinmeng163-oss/UEAtelier.git && cd UEAtelier`.
...
1. Clone to a short path: `git clone https://github.com/edwinmeng163-oss/UEAtelier.git C:\UnrealProjects\UEAtelier`.
