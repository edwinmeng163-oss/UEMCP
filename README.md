...
- The Chat panel `Task Atlas` button opens a Slate workflow view backed by `Saved/UnrealMcp/Tasks`. It supports local task extraction, search, detail inspection, rating, pinning, `To Skills`, `To RAG`, and `Make Tool` preview composite generation from ordered captured-argument stepRefs while keeping real replay deferred.
