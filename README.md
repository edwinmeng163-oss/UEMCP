...
- Task Atlas in Chat extracts local workflows from ActivityLog and supports task list/detail/rating/pinning.
- 154 registered MCP tools across actors, blueprint, editor, material, memory, scaffold, self-extension, skills, task-atlas, and widget categories.
...
- Chat 内的 Task Atlas 会从 ActivityLog 提取本地 workflows，并支持 task list/detail/rating/pinning。
- 已注册 154 个 MCP tools，覆盖 actors、blueprint、editor、material、memory、scaffold、self-extension、skills、task-atlas、widget 分类。
...
- Chat 内の Task Atlas が ActivityLog から local workflows を抽出し、task list/detail/rating/pinning を扱います。
- actors、blueprint、editor、material、memory、scaffold、self-extension、skills、task-atlas、widget カテゴリにまたがる 154 個の MCP tools を登録済みです。
...
- The Chat panel `Task Atlas` button opens a Slate workflow view backed by `Saved/UnrealMcp/Tasks`. v0.17 supports local task extraction, search, detail inspection, rating, and pinning. `To Skills`, `To RAG`, and `Make Tool` are visible placeholders for v0.18/v0.19.
